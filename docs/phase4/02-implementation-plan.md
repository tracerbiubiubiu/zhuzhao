...
| Wave | 线 | 内容 | 量级 | 出口标准 |
|------|----|------|------|----------|
| **P4-W0 启动批** | ④+拍板+安全 | 00 §4 四拍板项落地（03 号 D2/D4 定案、B13 定性、scope 枚举映射表回标）+ P2-14 `InsertPolicyEvals` batch_size 上限 + 00 §8.1 三项定性（F-23 审计防篡改重定性 / govulncheck 入 Makefile / 密码历史·邮箱重置归档定性）+ **二十一批拍板项三件**：B-7 golangci-lint 最低集（errcheck/gosec——review/08:110 登记链断裂第 4 例，回收进活跃追踪）；B-4 CI 触发分支加 phase4（现为 [dev,main,feature/**] 活跃分支零检查——分支列表一行改立即做，integration 作业另行拍板与 P2 CI 取舍关联）；B-5 覆盖率双档口径（现 18.7% 不含 integration 严重低估，趋势观察失真）+ **安全小批（十六批审计，全部代码实锤）**：① **SSRF（High，2026-09-22 所有者定根治法、十七批收口补全）**——zhuzhao Submit 的 callback_url 原样透传（taskrunner_service.go:57-71）+ **taskrunner 侧三口同收（十七批补：POST /v1/tasks 与 POST /v1/jobs 的 callback_url 均为 required 必填（server.go:79/:206）、PATCH /v1/jobs/update 指针可改（:273）、jobs 表存列且 cron 定时触发原样带出——只收 zhuzhao Submit 不够）**：**两仓四口全部不再接受该字段（传非空即 400——fail-fast 而非忽略+日志，忽略会让调用方以为自己的地址生效了），回调地址一律服务端定**（zhuzhao=self+/internal/jobs/callback；taskrunner 定时触发=配置里的 zhuzhao 回调地址，jobs 列保留、运行时读侧覆盖不迁存量）；POST /jobs 必填字段变更是 API 破坏性变更但消费方唯一=zhuzhao 代理端点同批改。**外部回调未来需求按 00 号雷达四档决策树走（拉模式首选，code 注册表仅第 2 档非默认）；内部再加服务=taskrunner 配置 action→base URL 映射（不是 code，注册表只给管理员登记外部 HTTPS 用）**；taskrunner 侧出站 deny 链路本地为纵深随批（跨仓）；② CreateRelation 侧信道（Med）——先查重后鉴权（service.go:524-535）不可见工单先泄 409 → 调序先鉴权后查重（并发正确性已由 000028 兜住）；③ 登出槽（Med）——body 空 device_id 静默落 default 槽、真实设备 RT 残留最长 168h（AT claims 无 DeviceID 可用）→ Logout device_id 改必填（空值 400；前端契约 01 §6 已承诺携带，零前端改动）；**P1 十一件快登（二十四批代理取证，实施时逐项复核）**：P1-8 Swagger 注解大面积缺失（仅 43 条 @Router/34 路径，users/roles/menus/audit/orgs 全缺）→**W2 首日前置**（codegen 前提不成立，补注解+make swag 否则类型只盖 ticket/task 域）；P1-9 wire_gen 含手改装配（gateway/rateLimit/BK-22 块无 provider）→W0 首日跑 make wire 验证；P1-2 activelist 验签前全量缓冲 body 共享 1GiB→无签名并发即 OOM，跨仓批修；P1-6 网关 60s WriteTimeout 截断大导出（客户端收 200 开头残缺 JSON）→W5 前修；P1-4 pgerr 缺 23514/22001 映射+P1-7 ErrRoleInUse 代码 403/文档 409 矛盾+ErrPolicyExists 死码无映射→随手项；P1-1 登录锁定仅账号维度（知工号可锁死任意用户）→P4-7 批加 IP 维度；P1-3 policyeval List 无封顶无 TTL（DB 长故障→安全机制连锁退化）→随手项 LTRIM+告警；P1-10 release 跑 debug 日志级→compose 注入 APP_LOG_LEVEL；P1-11 零 metrics（HTTP RED/pgxpool/队列积压）→P4-8 扩容；⑤RecordSubmit 记账失败静默（十九批：taskrunner_service.go:81-84 `_ = err` 显式空吞——受理成功但 zhuzhao 未记账且无日志，对账只能靠 task_id 人工拉齐）→ 最小修=slog.Error（含 task_id/actor）+runbook 观察注记；⑥**回调快照一致性（二十批纵深 Low-Med）**——ClaimCallbackRow 的 ON CONFLICT 分支只更新 status/claimed_at 不校验 action/params（job_submission_repo.go:92-140），Execute 按回调 body 的 action/params 调 handler（jobs_callback.go:33/:72）→ SK 泄露/taskrunner 被控场景可用已有 task_id 跑另一个已注册动作 → 冲突分支加「action 与 params 须与既有行一致，不一致 409」（快照列已有，纯校验逻辑）；④ gateway SK 弱密钥检查（Med）——D2-09 只盖 jwt.secret，`dev-gateway-sk` 明文在库且无检查 → 已知 dev SK 对齐 D2-09 全 mode 拒绝 | ≤3 天 | 四档 acceptance + 全门禁绿 + 安全四件各带负向测试（SSRF 恶意 URL 拒/登出空 device 400） |
| **P4-W1 菜单词表只读化** | ④ | [00 §2.4](./00-planning-inventory.md) 方案 B 全清单：删 3 写接口 + **迁移 000030+000031 两连号（十一批拆分：30=BK-18 整改、31=词表重排+按钮补种+绑定，序号即执行顺序；以下条目按所属迁移落位）**（**含 menu_apis 三行清理：system_menu × POST /menus·/menus/update·/menus/delete** + **system_menu_create/update/delete 三按钮行删除**（00 §2.4 清单原有、十批复审补登）——残留即孤儿绑定，catalog.go `AuditRouteCatalog` 双向对账启动即报）+ 死码随迁扩容（ErrMenuAlreadyExists 60001/ErrMenuHasChildren 60003/ErrMenuIsSystem 60004 + handler/errors.go 对应 HTTP 映射行 + **docs/api/errcode.md 删行** + **pgerr.go:26 菜单唯一约束映射处置（改通用冲突，防 raw 500）** + **errors_test.go:47 allErrCodes 码表断言同步（len 58→55）**；ErrMenuNotFound 60002 留——AssignMenus 预检在用）+ swag + 文档四处（07-menu 定位改写+契约句改写+**audit:list→audit:read 勘误**/§26.2 注记/11 号矩阵/迁移地图）+ **resource.go:66 注释随手修正**（「路由级已校验 ticket:create」→ B 后语义对齐「路由级=页面/按钮绑定含 POST /tickets」）。**+ B 案词表拆分（2026-09-21 所有者取 B，研究方案细化六点）**：① menu_apis 挂载点重排——页面行只留 GET、写端点挂既有按钮行，ticket/task/al 三域 + system/audit 同批统一（admin/superadmin 策略来自 000002 通配、rbac_service.go:307-312 跳过 menu_apis 路径=零影响——十二批行号勘误；模型统一后任意自定义角色天然支持只读）；② 补缺失按钮（**task_center 实为六写路由**——POST /tasks·/tasks/cancel·/tasks/retry·/jobs·/jobs/update·/jobs/trigger，十批复审补全）：`ticket_relation_btn(ticket:relation)`、`task_operate_btn(task:operate)` 挂 relations 与 cancel/retry；**`POST /tasks`（submit）归既有 `task_submit_btn`**（十二批点名补全，六写路由全数有主：tasks→submit_btn/cancel+retry→operate_btn/jobs 三写→manage_btn）；**/jobs·/jobs/update·/jobs/trigger 三端点显式归 `task_manage_btn`**；**ticket_type_manage 七写路由零按钮（000018 通篇无 type=3 行）——M1 已拍（2026-09-21）：补同码按钮 `ticket_type_write_btn(ticket:type:manage)`（**复用既有 permission 字面值、不新增 permission 定义**——十一批表述精确化；**粒度代价登记**：7 写端点挂一按钮=写权限全有全无，不能只给「改字段」不给「删类型」，内部系统可接受、非既有缺陷**）**——前端 W4/W5 关联/取消/重试按钮渲染同用新码；③ intent-preserving 迁移：非 admin 角色若现有 casbin 含写路由，按新形态补 role_menus 按钮绑定（防重存角色静默降权——AssignMenus 替换语义+每次重算 casbin；当前 operator/viewer 零绑定=实际空转，纯防御；**十批 P0-3：该分支无既有覆盖，acceptance 构造含写路由自定义角色验证一遍，或首次真实触发先 dry-run**）；④ 07-menu 契约句改写：「**页面菜单=该页读 API，写 API 挂对应按钮行**」（F-3 注释引用的原文同步改；模型复原=页面 GET+按钮写，F-3 本意「勾全即能写」保持——勾页面+全部按钮仍得全 API）；⑤ 不回退 F-3 已删过滤、不触发 catalog 拦截（AuditRouteCatalog 只对账路由↔menu_apis 集合、不校验 menu_type，catalog.go:95）；⑥ 规模修正（十一批）：**换挂 ≈47 行**（system 17/ticket 15=8 写+7 类型写/task 7=六写+GET /jobs/al 8）**+4 个元数据 GET 双页绑定**（GET /ticket-types·/:code/fields·/ticket-templates·/:code 在 ticket_list **保留** + ticket_type_manage 页面行**复制**绑定（menu_apis M:N 天然支持，casbin 按 (role,path,method) 去重）——**移走会断 operator 发起表单的 schema 拉取，只增不删**）；**GET /jobs 一并归 `task_manage_btn`**（「/jobs\* 管理面除外」语义完整性——只勾页面的角色不得见 job 定义，task_center 页面只留 GET /tasks/:id·/runs·/dead-letters）；ticket_type_manage 页 B 后 menu_apis=4 共享 GET（**绑页=仅导航与元数据读，写权限须绑按钮——推荐分配预设须体现「绑页+绑按钮」**）；**000031 内嵌「路由→按钮」逐行对照表（SQL 注释），down 精确还原，验收断言按表逐行核；对照表注记：system_org 组 POST /orgs/delete·/orgs/members·/orgs/members/delete 三行同时在 catalogExempt（SelfService 跳 Casbin）——换挂后生成的 casbin 规则永不参与 enforce，绑定仅声明用、真实边界在 L3（十二批装饰性注记）；**读按钮（\*_read_btn）重排后零 menu_apis 行=设计预期勿补**——读权限由页面承载、读按钮码仅作 UI 标记与静态路由入口，一补即破坏「页面=读 API」模型（十四批）**。**+ role_menus 绑定缺口修复（五批复审 P0）**：admin/superadmin 五菜单绑定 + acceptance 断言。**+ 旧迁移 down 补齐（二十四批 P0-4）**：000022 down 确定性失败（只删 2 code 不删 3 按钮，FK NO ACTION→23503，22 及以下回滚链卡死）；000018/24/25 同类（不清理 role_menus 条件性失败）——违反 up/down 成对铁律，照 000010 down 范式随 000031 同批补齐**+ 验收账号与权限基座（原「验收种子」，2026-09-21 拍板 P1+研究方案修正+十批 H1 采纳，十一批改名）**：**测试账号不进 migrations/（迁移落包括生产在内的每个环境，flag=false 已知凭证是比 admin123 更低的安全水位）**——E2E/Playwright setup 幂等建号：admin 登录后 POST /users 创建 operator/viewer 两个 `must_change_password=false` 账号（重复运行幂等）；**admin 凭据闭环：首跑经强制改密改到固定测试密码（存 E2E env）后续复用——改密即轮换，不闭环则二次运行登录失败**；运行时调 POST /roles/menus 分配（先例=phase2a:179），断言注意防抹种子；尊重 000002:11-12 角色描述既有契约（「默认未绑定权限，需管理员显式分配（只读菜单）后生效」——D2-45/#25 自然保持绿；且 viewer 描述本就承诺只读菜单=B 案兑现种子承诺）；「viewer=业务只读/operator=业务读写（/jobs* 管理面除外）」作为**推荐分配预设**写入 W3 文档（**十二批补三新按钮归属：`ticket:relation`→operator（关联工单=常规协作动作，与 comment/note 同类）；`task:operate`→operator（operator 提交的任务应能取消/失败重试；jobs 管理面仍经 /jobs* 归 task:manage 挡在 admin）；`ticket:type:manage`→admin 专属（**十四批码值勘误：按钮名 ticket_type_write_btn 的 permission 字面值=ticket:type:manage（000018:12），原写 type:write 系笔误——照错码实现按钮永不渲染，同类于十一批 button:{code} 坑**）；**viewer 预设=页面+读按钮（十四批）：工单详情静态路由入口按 `button:ticket:read` 放行（01 §3.2④），只勾页面不绑 ticket_read_btn 则 viewer 进不了详情页——「工单 list/read」预设显式含读按钮**——不补则 operator 在 W4 看不到关联按钮、W5 看不到取消/重试按钮，联调时易误判前端 bug**）；权限语义（所有者口径）：审计+system 域 admin/superadmin 专属，operator/viewer 均不绑。**+ BK-18 方法整改（单仓直接改，2026-09-21 拍板）**：5 端点 PUT/DELETE→POST zhuzhao 风格，**旧→新路径对照（十一批补全，替换原「等」字）**：`PUT /ticket-types/:code → POST /ticket-types/update`（code 入 body）、`DELETE /ticket-types/:code → POST /ticket-types/delete`、`PUT /ticket-types/:code/fields → POST /ticket-types/fields/replace`（整组替换语义，非 update）、`PUT /ticket-templates/:code → POST /ticket-templates/update`、`DELETE /ticket-templates/:code → POST /ticket-templates/delete`；+ menu_apis **api_path+api_method 双同步** + Go 集成测试更新 + swag + **standards §3-5 豁免条目删除**。**⚠ 同 path 跨 method 迁移红线（十二批发现、十四批计数勘误=四组）**：`/api/v1/ticket-types`·`/:code/fields`·`/api/v1/ticket-templates`·`/ticket-templates/:code` **四条 path** 跨页多行（各为 000010:143-146 ticket_list GET × 000018 类型页写；另 `/ticket-types/:code` 仅类型页 PUT+DELETE 两行无 GET 孪生不成对——漏 method 是两行撞同目标而非跨页提权）；**迁移 UPDATE 一律按 `(api_path, api_method)` 复合条件定位**。风险分级（十四批）：**真正静默提权只有 UPDATE 漏条件**（operator 的 GET 被改成 POST 写权限，AuditRouteCatalog 不报错）；DELETE 类误伤偏良性（删多→「路由存在无绑定」→missing_binding 启动拒启，fail-fast 兜得住）——紧张点在 UPDATE。出口反向断言：**ticket_list 必须仍持 4 条 GET 且不得持有 fields/replace 等写端点**。**迁移拆分（十一批采纳）：BK-18 URL/method 整改=000030，词表重排+按钮补种+role_menus 绑定=000031——migrate 按序号天然保证「先整改后重排」的钉死顺序，down 粒度减半；IW2 附件批按 A2 让位从 000032 起（规则本就覆盖）** | **≤5–6 天（二十四批 P0 扩容后=拍板定性 7 件 ~1.5 天+安全 9 件 3–4 天；可拆 W0a 定性/W0b 安全两段，开工时定）**（原核算：删接口 0.5+两迁移 1.5–2+BK-18 测试文档 1+intent/死码 1+验收 0.5–1） | 四档 acceptance+全门禁绿 + **部署原子性（H2）：migrate-up 与新二进制同一步连续执行——catalog 对账启动期 fail-fast 拒启（wire_gen.go:135），顺序错乱窗口内重启即宕机；运行中旧进程不重启不受影响（gate 只在启动期跑）** + BK-22 启动对账平 + 新 5 菜单 /user/menus(admin) 可见性对账平 + **新增只读回归断言：运行时给 viewer 绑 ticket_list 页 → GET /tickets=200 且 POST /tickets=403**（T7 反转增强）+ **operator 正向断言（十二批补）：operator 持 ticket:relation/task:operate 码且经其端点 200；/jobs* 仍 403**（十四批夹具注记：setup 须先 submit 一任务使处 pending、建两张工单，再断言 cancel/relation——顺序不能反） |
...
| **穿插池** | ①④ | P4-2 通知（本体含渠道配置持久化+管理 API；†配置页触发驱动后补——2026-09-21 拍板：配置能力现在加、页面后加）→ P4-3 字典（†system 域管理页）→ P4-4 导出（†前端导出交互，与 BK-21 同批）→ P4-7 验证码（前端登录插槽已在 01 §6 预留）→ P4-8 P2 小件（†panic 聚合/指标/对账极简查询页——2026-09-21 拍板要 UI，**随 P4-W5 audit 域同批交付**；候选追加：**工单聚合统计端点**——工作台首版按状态多拉 total 是权宜，终态化登记触发驱动，十批低优 2；登录日志独立表（与业务审计分表差异化留存，P2 遗留建议）一并入候选）→ P4-6 PAT（†个人中心管理页）（**P4-5 MCP 读侧已提排至 P4-W1/W2 并行窗口——2026-09-21 拍板 P2-7：纯后端不抢契约面，把快照/AuditRouteCatalog/权限码表暴露为只读 MCP tool，直接缓解单人带宽这一唯一高风险项**）；附件（IW2）独立窗口，启动先拍三决策点（**含补附件前端入口规格**——12-frontend 现无此件）；**④线漏排四行同池（2026-09-21 拍板 P5 分件处置）**：**cron 收归+回调长任务批（二十一批扩容并入 B-1/P1）**（**2–2.5 天**含 taskrunner 跨仓退役+**B-1 长任务双执行修复**：修法 c+d=audit_archive 去 for 循环改「单批一回调」（工作量有界，天然消 WriteTimeout 60s 冲突——服务端 60s 使 >60s Handle 无法 2xx 而 taskrunner 超时必重试，长任务回调重试是必然非假设）+「Handler 必须可重入」从注释升格 registry 契约+补「长任务×并发重试」集成测试（现 8 并发测试皆短任务测不出该窗口；备选 a 心跳续租/b 按动作租约不取）——七批复审估时修正：zhuzhao 侧先行可交付，taskrunner 侧 job 定义退役并行尾随不阻塞；**退役 smoke checklist（taskrunner 无等价 acceptance 门禁，十批 P1-6）；/jobs* 端点终态冻结=保留（W1 重排前置条件）**；头位——所有者质疑成立：审计数据与归档内务全在 zhuzhao 本地，闹钟外置 taskrunner 属既成依赖；方向=收归进程内 ticker + PG advisory lock 单实例锁 + 配置开关，taskrunner 侧 job 定义退役）/文档治理批（~1 天，**提前：P4-W1 收口后立即执行，不等穿插间隙**——00 §6 回标搭 W1 便车，ADR 补篇+两大文档拆分随本批；**子项全清单见 00 §2.4 文档治理行（含 S-2 策略库标注/S-6 版本滞后/phase2-00:361 更正——十批复审补指针）**）/随手项打包（**1.5 天**，尾位；角色复制按钮随 P4-W3 角色页；**十六批正确性六件并入**：评论插入 FK 映射（现易 500，对齐 MapForeignKeyViolation）/分派 assigned_to 用户存在性校验（裸 BIGINT 无 FK）/ListComments 分页/CreateRelation 400 文案分流/**org Create·模板 Create 的 org_path 事务内 FOR SHARE（BK-11 同型残留——工单 Create 已修，两处未及）**/UpdateStatusTx 失败定性注记；**二十一批两件**：standards 迁移规范补「大表索引 CREATE INDEX CONCURRENTLY+不可在事务内执行约束+迁移前行数/锁窗口预检」条目（49 处索引零 CONCURRENTLY、全 docs 零提及在线迁移——B-2，当前量小无感属未来定时问题）/审计写入失败可观测升级登记（现仅 slog.Error 无重试/指标/告警——B-6 并入 P4-8 候选）；**二十批五件**：认领窗重复副作用（claimed_at 10 分钟过期可重认领+再执行——audit_archive 单批超 10 分钟会重复导出；评估调大窗口或按批续租，fence 只防状态覆盖不防双跑）/BindOrgRole 预检与插入两步无锁（org_service.go:167-200，与成员插入已修的双侧锁同型）/errcode.md 10008 固定写 503 但实际双态（网关·taskrunner 上游不可达=502+10008，gateway.go:102/handler:283；仅 Redis fail-close 与停用=503——文档改双态注记）/callback_url+self_base_url 双空误报 502「服务不可达」（实为本机配置缺失，taskrunner_service.go:25 注释自认）/不可重试回调 err.Error() 拼进 409 文案（内网端点仍可能带 SQL/路径细节出网到 taskrunner 日志））/activelist 契约整改②（**✅ 2026-09-22 拍板：整改不豁免——改 W5 前置小批（~1 天两仓）**：activelist 三端点 POST 体携带 typeName/action（/deprecate·/schema·/restore 改 zhuzhao 风格）+ zhuzhao 一笔迁移同步 000024 种子 menu_apis path + 网关路由匹配 + 验收断言；**时机=W5 动工前（前端 al_types 页尚未开发，改完零返工；P4 收尾才改=最差时机，前端已按旧契约写完要返工）**；standards 豁免诉求就此消失；**同批顺手：standards §3 增补 taskrunner /tasks/cancel·/retry 动词段豁免注记**（同构违反 §3-2 但性质不同——纯内部 M2M 语义动词段，B 案/W5 已按此形态设计，改=推翻已定契约收益低，业界 RPC-over-HTTP 常态））+ **B13 执行本体**（0.5–1 天矩阵——**不走间隙规则：W1 收口后立即执行（与文档治理批同窗），W2 启动前必须完成**；W0 仅定性，七批复审指认「穿插池定位 vs W2 前硬时限」自相矛盾，已消解）。④线各件加「P4 收尾前必须清」软时限。†=含前端联动面，随本批同步交付（页面工作量已在量级内），**交付时点不早于其依赖的前端基座 Wave：字典页≥W3、PAT 页≥W2（profile 页扩展）、导出交互≥W5、P4-8 UI=W5——九批复审钉死，替换原「随最近 Wave」歧义表述** | 各 0.5–5 天 | 每批独立过门禁 |
...
- **迁移号**：000030 起。**P4-W1 占 000030（BK-18 整改）+000031（词表重排）两连号（十一批拆分）**；**000032/000033 为 P4-2 与 IW2 的待占位、A2 裁定归属（十四批改「待占位+预期」，替代十二批写死的 IW2=32/P4-2=33——与 A2 及 §6 #7 穿插序自相矛盾：默认序 P4-2 在穿插池首位、IW2 须先过三决策点更晚启动，故预计 P4-2 占 32+33 两连号（本体/事件消费位）、IW2 顺延 000034；IW2 先启动则整体对调）**。
...
| 17 | **十四批计数与占位勘误（2026-09-22，全部核验成立）**：①红线「三条 path」实为**四组**（/ticket-types·/:code/fields·/ticket-templates·/ticket-templates/:code 各有跨页 GET×写；/ticket-types/:code 仅类型页两行不成对——漏 method 是撞目标非提权）；**风险分级：真静默提权只有 UPDATE 漏条件，DELETE 误伤被 missing_binding fail-fast 兜底**；②P4-2 触发源锚点补全——000010:70 注释承诺「迁移 000021 加 processed」但 000021 已被 job_submissions 占用（承诺编号被 A2 挪用），P4-2 预期占两连号（本体+消费位）；③§4 迁移号改「待占位+预期」（P4-2 预期 32+33、IW2 顺延 34，A2 裁定——替代十二批写死赋值，消除与 §6 #7 穿插序的矛盾）；④**码值勘误：预设段 ticket:type:write 系笔误，实为 ticket:type:manage（000018:12）——照错码实现按钮永不渲染**；⑤viewer 预设补「页面+ticket_read_btn」（详情静态路由入口按 button:ticket:read 放行，只勾页面进不了详情）；⑥对照表注「\*_read_btn 零绑定=设计预期勿补」；⑦operator 断言夹具前置（先建 pending 任务+两张工单）；⑧01 §11:218 镜像 000030→000031 | §2 W1/§4/§5.5#15 + 01 §11 |
| 18 | **十五批豁免清偿拍板（2026-09-22，所有者「按建议改，W5 前」）**：① activelist 契约整改② 定**整改不豁免**——W5 前置小批 ~1 天两仓（三端点 POST 体携带+zhuzhao 迁移同步 000024 种子+网关+验收；时机理由=前端 al_types 未开发零返工，P4 收尾改=最差时机）；② **新发现同构项：taskrunner /tasks/cancel·/retry 亦违反 §3-2「动作进 URL」——主动登记豁免**（内部 M2M 语义动词段，B 案/W5 已按此形态设计，改=推翻已定契约，随 ① 批写 standards 注记）；③ **时机敏感债归位：org 删除守卫补查 org_roles（BK-12 残留）→W3 批内顺手件 0.5 天**（FK 兜底报错在删除 UI 上线后体验差）；④ 其余豁免盘点维持：探针信封/文件规模存量/权限码三段形（:86 已注记「改码=破坏既有角色绑定」）=设计内合理豁免不改；BK-18 豁免已随 W1 整改消亡 | §2 W3/穿插池/§3 + 00 §2.4 |
| 19 | **十六批安全与正确性审计（2026-09-22，双代理核验全实锤）**：High ①SSRF（callback_url 透传）/②FireDue 无互斥（仅多实例→挂多实例触发线）；Med ③CreateRelation 先查重后鉴权侧信道/④登出空 device 落 default 槽（AT claims 无 DeviceID→改必填）/⑤工单 Update 整行覆盖丢更新（→W4 COALESCE）/⑥org·模板 path 事务外快照（BK-11 同型→随手项）/⑦CORS（已有 B7 无动作）/⑧gateway dev SK 无弱密钥检查；Low ⑨评论 FK/assigned_to 校验/ListComments 分页/400 文案等六件（→随手项）；⑩任务代理 L1 无提交者范围——**✅ 口径已拍（2026-09-22 所有者）：任务中心=共享作业队列、跑的任务全可见（viewer 保留 task:read），UI 加「只看我提交的」筛选（W5 随批：taskrunner submitted_by 过滤+代理透传）；params 敏感性由 E-⑤ 触发项既有登记覆盖**。①-④入 W0 安全小批 | §2 W0/W4/穿插池/§3 + 03 号 |
| 20 | **十六批二轮拍板（2026-09-22 所有者）**：SSRF 修法升级——拒绝用户级 callback_url（生态内唯一合法回调=self，字段无正当用途；传非空 400）替代 allowlist，**根治而非缓解**；外部回调未来需求=预注册 code 模式触发驱动登记。任务读口径落定——共享队列全可见（viewer 保留 task:read）+「只看我提交的」筛选（W5 随批 **仅 ListRuns** submitted_by 过滤跨仓 ~0.5 天——详情勿加范围参数，十八批修正）；#19 行⑩「待拍」就此作废 | §2 W0/W5/§3 |
| 21 | **十七批 SSRF 收口补全（2026-09-22，四点全采纳）**：①code 模式降档——W0/行 20 原文「未来外部回调=code」与 00 号雷达决策树（拉模式首选）不一致，已同步改写；②**内部再加服务≠code**——taskrunner 配置写 action→base URL 映射（调用方只传 action），注册表只给「管理员登记外部 HTTPS」用（雷达补第 0 档）；③**taskrunner 三口同收**——POST /v1/tasks 与 POST /v1/jobs 的 callback_url 均 required（server.go:79/:206）、PATCH /v1/jobs/update 指针可改（:273）、jobs 列 cron 定时触发原样带出——两仓四口全收、jobs 列保留运行时读侧覆盖不迁存量、POST /jobs 必填变更消费方唯一（zhuzhao 代理）同批改；④**400 定案**（废弃「忽略+日志」选项——忽略会让调用方以为地址生效） | §2 W0 + 00 §3 |
| 22 | **十八批决策摘要核验（2026-09-22，摘要与已落档主体一致，三处对齐+两新约束）**：①W5 submitted_by **仅 ListRuns**（详情按 task_id+task:read 即可——修正 #20/W5 行误含 GetTask）；②03 号 S13「callback_url 必填」随 W0 改拒收（前端不传）；③16 号 E-⑤ 补 2026-09-22 口径一句；**新约束落 00 雷达**：不做 push+pull 双执行引擎（拉模式=读 API 消费方式非第二执行器）+未来 webhook payload=通知型（业务细节走 GET，webhook 不当 RPC）；16 号「不对外推送」语义澄清（不对业务方 POST≠取消内部 callback） | §2 W5/§5.5#20 + 03 S13 + 00 §3 + 16 号 E-⑤ |
| 23 | **十九批全生态对照研究（2026-09-22）**：净新增四件已落——①**11 号最小回填**（头部日期/分支/Phase 4 规划批注记/导航树/迁移号谱系/TC1 行回标——修复「AGENTS 回填纪律」欠账，评估 F 组点名）；②**design-decisions 补 §26.6 任务回调与外部集成口径档**（SSOT 落位，评估「§25 回调决策未入 dd」）；③**RecordSubmit 吞错登记**（`_ = err` 空吞→W0 ⑤ slog+runbook）；④authz.md:253 示例防误读注记（M-E 接线形态≠已接线，S-2）。**评估三断言不采纳**：16 号「OrgMember 首个消费者=M-E 与代码不符」零命中（表述在 authz.md 系示例代码，S-2 已如实标注）；「Builtin 未接线」=已知登记（00 §2.4 文档治理行含 S-2）；Swagger callback 注释=W0 swag 重生成自然覆盖。其余 A–H 组均为已登记项复述 | 11 号 + dd §26.6 + §2 W0 + authz.md |
| 24 | **二十批四轮交叉核对（2026-09-22）**：净新增已落——①**回调快照一致性**入 W0 第⑥件（ClaimCallbackRow 冲突分支不校验 action/params+Execute 按回调 body 执行→SK 泄露场景可挪用 task_id 跑别动作；冲突分支加一致性校验 409）；②随手项+五件（**10 分钟重认领窗重复副作用**——评估调窗或按批续租/BindOrgRole 两步无锁/errcode.md 10008 单写 503 实为 502·503 双态/双空配置误报 502/409 文案泄 err.Error()）；③11 号 :48「M-HR(预留) 交付」残留句修正；④query 不进 AK/SK 签名=包注释既有设计前提（内网可信），登记观察不动代码。**降级两项确认**：禁用用户 AT 残活=D2-05 既定补偿路径非新洞；组织 Create path=随手项已登记（本轮再确认实锤）。其余与十六~十九批登记一致 | §2 W0/穿插池 + 11 号 |
| 25 | **二十一批盲区报告核验（deliverables/zhuzhao-blind-spots-2026-09-22.md）**：①**§3 过程性事故证实并已恢复**——eab4535 虚标勘误 4 处（dd §9.4/§12.6/§13.6+09:515 hooks.go）被 25bda2e 静默回退（git log -S 双向实证），本轮按 eab4535 原文全部恢复+恢复处加「曾被误回退」注记；**教训两条入册：提交信息必须披露删除类改动；批量脚本落盘后必须 grep 复核目标仍在**；②B-1（P1）实锤且链条更深（WriteTimeout 60s×taskrunner 超时必重试×for 循环至清空×无心跳）→并入 cron 收归批改名「cron 收归+回调长任务批」2–2.5 天；③B-2 实锤（49 索引零 CONCURRENTLY）→随手项规范条目；④B-4/B-5/B-7 实锤→W0 拍板项+3；⑤B-6→P4-8 候选；⑥**B-3 误报不采纳**——000020:20-22 明有 idx_pel_created/idx_pel_actor_created/idx_pel_trace 三索引，报告「零索引」与事实相反（其工具检索失真）；⑦§5.1（L2 仅 ticket）=B13 已盖、§5.2 恢复 runbook→00 雷达、§5.3 判定日志无读 API=十三批已核 | §2 W0/穿插池 + 00 §3 + dd/09 恢复处 |
| 26 | **二十二批前端-后端对齐复核（2026-09-22，八条全实锤，三条补进 01）**：①**乐观锁契约前端缺位**——用户/组织 UpdateUserRequest/UpdateOrgRequest version `binding:required`（类型/模板为可选 *int CAS），冲突码 **10006**（utils ErrConcurrentModification「数据已被修改」，zhuzhao 别名透传——首查 errcode.go 本体未中系其在 utils 段）→ 01 §5 表单回传 version+10006 重拉详情、§3.3 白名单补 10006；②01 §8-W4/W5 落后于 02/03 → 补评论无分页「全部+上限提示」/设计器「图可配但可点动作仅五种」注记/W5 四点（callback 不传+params 勿填密钥提示+只看我的仅 ListRuns+导入经网关 1MB）；③工作台 6 次列表/姓名回填/顶栏无角色/dev CORS 联调期成立=已知登记项的确认复述。评审定性认同：按现稿开工、W2 前置=W1 菜单绑定+首日锁版本跑通 vue-tsc 不过即换备胎 | 01 §3.3/§5/§8 |
| 27 | **二十三批前端遗漏扫（2026-09-22，12 条核验全实锤，03 加五段+01 三处）**：①**429 双语义**——20006 账号锁定 vs **10007 全局限流**（utils ErrTooManyReqs+Retry-After 头 ratelimit.go:105）→01 errorBehavior 补 10007+S16 分流；②**角色 update version 亦必填**（role_request:36）→§5 点名补齐用户/组织/角色三件套；③任务 optional dept 首版写死不传+jobs Tab 按 task:manage 显隐（operator 不见）；④S10 评论作者勿 N+1（随 W4 批或批量）+**勿规划事件轴**（ticket_events 无读 API）；⑤S8 G2 400 挂表单项+regex 客户端预检；S12 fields 全量替换保存前危险确认；S14 sensitive 脱敏展示；S15 request_body 折叠；S7 强调名册端点=W3 阻塞；P4-9 进 01 §8 表显性化（dev proxy≠生产形态）。三四组为已知项确认复述 | 03 S7/S8/S10/S12–S16 + 01 §3.3/§5/§8 |
| 28 | **二十四批 P0×5 全实锤（2026-09-22 逐项代码核验）**：P0-1 三 SK 公开兜底+只查空不查弱+攻击链（公开 SK→回调 INSERT 采信→retention_days 覆盖=未授权删审计+无 nonce 可重放）→W0-④扩三件套；P0-2 org:% LIKE（org_delegation.go:142，注释 P2-2 自知）授任一 org 按钮=9 端点全局 org admin（BindOrgRole 链式提权）→W0-⑦ 显式白名单按端点细分、W1 前置；P0-3 AddMember 缺 scope=all 守卫（对照 SetMemberScope:205）→W0-⑧；P0-4 000022 down 确定性失败（FK NO ACTION 23503）+18/24/25 同类→W1 down 补齐随 000031；P0-5 工单 Create 零归属校验+assigned_to 暴露（ticket_request:11）→W0-⑨（BK-20 口径）。**W0 扩至 ≤5–6 天（可拆 W0a/W0b）**；P1 十一件快登（P1-8 Swagger=W2 首日前置最关键/P1-9 wire 验证 W0 首日/其余分流跨仓·W5 前·随手·P4-7·P4-8）；P2 清单（无角色用户重置密码卡死/AT 过期无法登出 RT 残留/关联无解除端点/无 .dockerignore/acceptance 硬编码 Redis 密码/PG·Redis 弱密码兜底等）登记随手项+触发观察，详单见评审原文 | §2 W0/W1 + 穿插池 + P4-7/P4-8 |
