...
| 19 | **十六批安全与正确性审计（2026-09-22，双代理核验全实锤）**：High ①SSRF（callback_url 透传）/②FireDue 无互斥（仅多实例→挂多实例触发线）；Med ③CreateRelation 先查重后鉴权侧信道/④登出空 device 落 default 槽（AT claims 无 DeviceID→改必填）/⑤工单 Update 整行覆盖丢更新（→W4 COALESCE）/⑥org·模板 path 事务外快照（BK-11 同型→随手项）/⑦CORS（已有 B7 无动作）/⑧gateway dev SK 无弱密钥检查；Low ⑨评论 FK/assigned_to 校验/ListComments 分页/400 文案等六件（→随手项）；⑩任务代理 L1 无提交者范围（**口径待所有者拍：viewer 预设 task:read 去留——留=viewer 可见全队列含他人 params；去=任务中心 viewer 不可见，零后端改动**）。①-④入 W0 安全小批 | §2 W0/W4/穿插池/§3 + 03 号 |
