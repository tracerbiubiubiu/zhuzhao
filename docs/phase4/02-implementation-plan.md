...
| 20 | **十六批二轮拍板（2026-09-22 所有者）**：SSRF 修法升级——拒绝用户级 callback_url（生态内唯一合法回调=self，字段无正当用途；传非空 400）替代 allowlist，**根治而非缓解**；外部回调未来需求=预注册 code 模式触发驱动登记。任务读口径落定——共享队列全可见（viewer 保留 task:read）+「只看我提交的」筛选（W5 随批 submitted_by 过滤跨仓 ~0.5 天，ListRuns 现无此参数）；#19 行⑩「待拍」就此作废 | §2 W0/W5/§3 |
