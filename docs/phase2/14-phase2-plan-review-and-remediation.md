...
| **设计文档落档-真前置（2 项）** | 11 号 §2.2 Q1 公式 + §3 Q3 错误映射（可选带过 Q5）| `11-authz-architecture-review.md` §2.2 / §3 | P2（**开工门禁阻塞项，见 §0.3 Step 1**，原列类别 2 属归类矛盾） | TicketResource.Authorize 语义不能写错：三层合取/析取结构（Allow ⟺ L1 ∧ (L3∨L2)）、属主短路顺序、DB 错误 fail-closed 映射（503/500/403）。11 号文档已给出建议值，拍板确认 ≈20 分钟。 | 采纳 11 号 §2.2/§3 建议值写入 architecture.md §4.1 同一节；P2-D6 同步挂入 00 §1。Q5 缓存禁令 1 行可一并带过。 | **2a Step 2 编码前（§0.3 Step 1）** |
...
| **P3-5** | P3 | 2b 体量偏大（5 Step / ~14 人日），含 HR Sync（最大不确定块 RK-3 概率=高）+ 虚拟组/scope + 存储 + 认证增强 | 00 §4 已标 Step 6 与 7 并行（批次 γ），可接受。但建议在 2b 启动时再评估一次是否需拆分为 2b-1（组织+scope）和 2b-2（存储+认证） |
...
| **P3-10** | P3 | Phase 1 的 27 条验收用例仅「人工跑回归」，**未固化为 CI/MR 强制门禁**。RK-10 风险登记表提及每里程碑必跑回归，但未声明 CI 必须 -race 全绿 + 27 用例不降。Phase 2 改动破坏 Phase 1 时无自动拦截 | 在 00 §5「工程流程规范」下补一条「CI 强制门禁：`make compile vet test -race` + `acceptance-phase1.sh` 全绿，否则 MR 不可合入」。与 Phase 3a 的「集成测试自动化」呼应，但必须前移到 Phase 2 开工即生效 |
...
| **P3-14** | P3 | Phase 2 期间 HR Sync 失败告警缺失（07 P3-6）。Phase 3a 才做可观测性，Phase 2 只有 slog + request_id，HR Sync 是定时任务，静默失败数天不被发现的风险真实存在 | 2b HR Sync 补简单失败告警：slog.Error 记录 + 可选 webhook（或返回非 0 让 cron 告警）。失败告警写入 `hr_sync_runs` 对账表（已有）。 |
...
| P3-10 | Phase 1 27 用例未固化为 CI 强制门禁 | 检查点 3（05 P-8） | 00 §5 补 CI 门禁（10 分钟） |
...
| P3-10 | Phase 1 27 用例未固化为 CI 门禁 | Part 3 §3.3（05 P-8） | 00 §5 工程流程补 CI 门禁硬约束（10 分钟） |
