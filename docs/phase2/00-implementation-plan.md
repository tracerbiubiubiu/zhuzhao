...
| **BK-22** | **路由↔menu_apis 一致性对账**：menu_apis 迁移行与 router.go 实际路由靠人工对齐，接错=混权（有路由无码=裸奔、有码无路由=死策略）。RuoYi-Go 双项目核验实锤同款痛点（Kun 版 120 条路由权限串硬编码在 container.go 与 sys_menu 种子人肉同步；lostvip 版 PermissionMap URL 不含 method） | 2026-09-08 RuoYi 借鉴核验提出。**随批次 B 实施**（activelist API 入 menu_apis 正需此闸）：启动期/架构测试用 gin 自注册路由清单 vs menu_apis 表双向 fail-fast；Form B 上报式能力目录落地时转上报报文校验器（design-decisions §26.2） | 🔶 **已登记（2026-09-08），随批次 B，~半天** |
