...
| **BK-12** | `org_roles` / `roles.parent_id` 写侧管理接口缺失（BFS 源 2/3 只有读侧） | 000012 建表后仅 BFS 三源展开消费（role_repo.go）；handler/router 零端点，验收脚本用 psql 直插搭场景（acceptance-phase2b.sh:177）；`parent_id` 管理入口按 000012 注释承诺「随 2c 角色管理补齐」未兑现。**必要性评估（2026-08-31）**：机制符合业界主流实践（Entra ID / Google Groups / Keycloak / AWS Identity Center 组驱动赋角）；当前 fail-inert（无人能写即无行为），价值随成员高频变动兑现。**非纯 CRUD**：org_roles 绑出的是全局 Casbin 角色（进 L1），防提权须按「发全局角色」设计（复用 user_roles assign 优先级规则 + 全局管理员限定） | **✅ 已实施（2026-08-31，IW3 附带）**：org_roles 绑定/解绑/列表三端点（仅全局管理员——org_roles 赋出全局 Casbin 角色，影响面=全组织成员；系统角色禁绑）；parent_id 写侧（Create/Update + 单调校验 child.pri ≤ parent.pri + 环检测 + 系统父禁用 + 自引用拒绝；ClearParent 清除）+ TestBK12_OrgRoleBinding / ParentIDRules。真实需求诉求出现时再评估 UI 入口；**parent_id 设置规则（2026-08-31 拍板，**方向修正见同日变更记录**）**：校验 **`child.priority ≤ parent.priority`（子不弱于父）**——BFS 绑子得父，设计语义为"子权限 ⊇ 父权限"，故只允许强子继承弱父（增益无提权）；若放行弱子继承强父（旧方向 parent ≤ child 恰好放行），admin 即可设 `child(10).parent = rank1` 绕过"不能赋 priority<10 角色"的防线，纯提权。**收尾修复（2026-09-01）**：① BindOrgRole/ListOrgRoles 加 org 存在性预检（对齐 AddMember 模式）——org 不存在/软删 → 404 ErrOrgNotFound（修复前：Bind 落 FK 23503 裸 500、List 空列表冒充 200、软删 org 可静默绑成脏数据）；② BindOrgRole INSERT 错误分支接 `MapForeignKeyViolation`（兜预检与写入间并发窗口 → 400）；TestBK12_BindOrgRole_OrgGuard（不存在/软删/repo 层 FK 映射三断言）。**已核实不修**：UnbindOrgRole（org 不存在时 DELETE 0 行本就返回 404 ErrNotFound，语义可接受）；环检测 O(depth) GetRoleRef 循环（≤32 次查询，管理端低频 + 深度 32 为防御上限，递归 CTE 有 GetEffectiveRoleCodes 先例可随时照抄，收益为负不动）。**残留登记**：org 删除守卫（Delete/DeleteVg）不查 org_roles——先 Bind 后软删留下孤儿绑定行；读侧 ListOrgRoles/BFS 均已过滤软删 role、且软删 org 必无成员（Delete 守卫），无权限效果，纯数据卫生，触发条件 = org 恢复/HR 同步对账功能启动时一并清 |
...
| **BK-20** | 禁删有未结工单的组织：`Delete` / `DeleteVgWithOwnerCleanup` 只守卫 children/members，不查工单——软删后遗留工单对原 owner / 祖先 owner 保留委托读+写（`delegatedVisibilitySQL` / `IsOrgAdminOrOwner` / `IsAncestorOwner` 三处无 `deleted_at` 过滤；vg 删除路径系统性产生 ghost） | 2026-09-02 外评实证 + 语义拍板（design-decisions §21）：活工单=需管辖 → 禁删；已结工单 → 委托残留=档案连续性，SQL 不加过滤、登记不修。**方案**：两删除函数加 `status <> 'closed'` 工单计数（子组织禁删已保证范围=自体）+ 新错误码 ErrOrgHasOpenTickets（409，语义「先结单或转派」，对齐 BK-18 类型有单禁删先例）+ 负向集成测试（有未结工单删 → 409；结单后删 → 200；软删后 owner 列表仅剩已结档案可见）+ **acceptance 删组织用例核对适配**（D6 等删 vg 场景若留未结工单需先结单） | **✅ 已实施（2026-09-03）**：两删除函数同事务加 `status <> 'closed'` 工单计数 + `ErrOrgHasOpenTickets`（50013→409，errcode.md 已登记）+ 集成测试 TestBK20（未结 409→结单后删 200）+ acceptance D6 段补 3 断言（open ticket→409/50013/结单后 200）；acceptance 用例核对结论=现有删组织用例与守卫无冲突（D6 的 VG2 无工单）。全门禁绿（lint/单测 13 包/集成 13 包/acceptance 四档）。语义 SSOT = design-decisions §21 |
| **BK-21** | **IW4 护栏未泛化**：`ticket_repo.List` 的 fail-closed 哨兵 + `TestGuard_TicketRepoListCallSites` AST 守护仅覆盖工单一处；IW4 修复的是「已知调用点」，未来新资源接 L2（Builtin 或手写 Resource）时漏接 `GetFilter` 仍 = 静默全量（旧债模式：GetFilter 显式调用零护栏） | 2026-09-08 OPA/ReBAC 迁移复核（11-authz §9.3）代码侧清点。**触发条件 = 首个新资源接 L2 / 导出功能出现**；导出为高危场景（绕过分页全量拉取）必须人工接 L2。**实施选项**：① 哨兵泛化为 registry 层通用机制（List 查询构造强制经 Filter）；② AST 守护扩展覆盖全部 repo.List 调用点；**实施蓝本 = Ruby Pundit `Policy::Scope` 结构惯例**（业界已验证的结构性防漏调形态，2026-09-08 注记） | 🔶 **已登记（2026-09-08），触发驱动** |
| **BK-22** | **路由↔menu_apis 一致性对账**：menu_apis 迁移行与 router.go 实际路由靠人工对齐，接错=混权（有路由无码=裸奔、有码无路由=死策略）。RuoYi-Go 双项目核验实锤同款痛点（Kun 版 120 条路由权限串硬编码在 container.go 与 sys_menu 种子人肉同步；lostvip 版 PermissionMap URL 不含 method） | 2026-09-08 RuoYi 借鉴核验提出。**随批次 B 实施**（activelist API 入 menu_apis 正需此闸）：启动期/架构测试用 gin 自注册路由清单 vs menu_apis 表双向 fail-fast；Form B 上报式能力目录落地时转上报报文校验器（design-decisions §26.2） | **✅ 已实施（2026-09-09，068207e+后续）**：internal/router/catalog.go（AuditRouteCatalog 双向 + 豁免集 + 网关前缀跨仓边界）+ wire 层启动 fail-fast（缺口拒启列全清单）+ 集成发现跑（真抓出 audit/logs 权限面缺失 → 000025 补注册）+ 纯函数单测 2 |
...
| 2026-08-31（方向修正） | 外评纠错：继承单调方向反了——绑子得父（BFS 向上），正确单调为 **`child.priority ≤ parent.priority`（子不弱于父）**；我落档的 `parent ≤ child` 恰好放行"弱子继承强父"= 纯提权旁路（admin 设 child.parent=rank1 即可绕过赋角防线）。00 §9 BK-12 与 05-role 注记已翻转并补机制论证 |
| 2026-08-31（priority 收敛） | 外评两条拍板落地：① tickets.priority 刻度收敛 1–4（Create/Update/模板默认三路径校验，0=缺省 3；存量已清点全在 1–4），TestPriorityScaleValidation；② priority×parent_id 双轨规则拍板（父角色等级 ≤ 子角色，随 BK-12 落地），BK-12 行与 05-role 注记 |
...
| 2026-08-31（P1-1/P1-2 核验修复） | 外评五条核验：**P1-1 属实**（user 模块守卫=直接角色、org 模块=展开集，语义分叉真实但无提权路径——系统角色禁作 parent/绑定已堵死）→ 处置：方案②落档锁定（管理守卫只认直接角色=防档位抬升设计选择，TestBK12_DirectVsExpanded 固化）；**P1-2 属实并修复**：UpdateRole 补子角色降级守卫 EnsureChildrenPriorityAllowed（新值 < 任一子角色 → 400，堵死"降父抬子"继承提权链）+ TestBK12_PriorityDemotionGuard；P2-1 注：CTE UNION 去重天然终止（"503"说法不成立）已落注释；P2-2 org:% LIKE 约束已落注释；P2-3 入随手项 |
| 2026-08-31（BK-12 实施） | **BK-12 落地**：org_roles 绑定/解绑/列表三端点（仅全局管理员、系统角色禁绑）+ roles.parent_id 写侧（Create/Update + 单调校验 child ≤ parent + 环检测 + ClearParent）；TestBK12_OrgRoleBinding（含 BFS 源 2 生效/失效断言）+ TestBK12_ParentIDRules（单调/自引用/环/系统父/链式 BFS/ClearParent）全绿 |
| 2026-09-01（BK-12 收尾） | 外评两条 minor 核验后修复：① BindOrgRole/ListOrgRoles 加 org 存在性预检（不存在/软删 → 404 ErrOrgNotFound；修复前 Bind 落 23503 裸 500、List 空列表 200、软删 org 静默绑脏数据）；② BindOrgRole INSERT 接 MapForeignKeyViolation 兜并发窗口 → 400。TestBK12_BindOrgRole_OrgGuard。Unbind/List 原判核验（Unbind 本返 404、环检测 O(depth) 评估成立，均不修）；残留登记：org 删除守卫不查 org_roles（孤儿绑定行，读侧已挡，随 org 恢复/HR 对账启动清理） |
...
| 2026-09-03（BK-20 实施） | **BK-20 落地**：`OrgRepo.Delete` / `DeleteVgWithOwnerCleanup` 同事务加未结工单计数守卫（`status <> 'closed'`）+ `ErrOrgHasOpenTickets`（50013→409，errcode.md 登记）+ TestBK20 集成测试（未结 409 / 结单后删 200）+ acceptance D6 段 3 断言；现有删组织用例核对无冲突。工单封版期的数据安全修复（§23），全门禁绿。插曲：acceptance 首跑 3 FAIL——dev 服务（:33333）为陈旧二进制，重编译重启后全绿（陈旧进程故障模式再现，已记 [[验收脚本教训]] 同类） |
| 2026-09-08（OPA/ReBAC 复核） | 项目所有者提问「是否迁移 OPA/ReBAC」→ 文档+代码双线复核，**维持不迁移**（触发表六条零命中；OPA 不对口 SQL 行级判定形态；ReBAC 成本 = tuple 双写管道 + 事务一致性损失，且与 activelist 行级自治设计冲突）。复核记录落 11-authz §9；§9 登记 **BK-21**（IW4 护栏泛化，触发驱动）；Casbin Watcher（W1 既有归口）与 L1 缓存（09 号既有归口）确认不重复登记；11 §6/§8（BK-21 行 + B12）与检查单 §2.2 同步 |
| 2026-09-08（RuoYi-Go 双项目借鉴核验） | 对比 Kun-GitHub/RuoYi-Go（iris+gorm 六边形）与 lostvip-com/ruoyi-go（gin+lv_framework，RuoYi v4 服务端渲染）：两项目均只完成 L1+管理台 UI——Kun 版 L2 仅 4 处接口且 fail-open+多角色并集越权，lostvip 版 data_scope 零运行时消费（参数驱动行级）。**可借鉴**：① **BK-22 对账（新登记，随批次 B）**；② data_scope 自定义部门集实现形态（仅借形态：锚点授予表 UNION 锚点集，勿抄角色派——与 BK-14 冲突，11-authz §1.2 补注记）；③ 在线用户管理面（随手项，触发驱动，0.5–1 天）。**反面印证**：L2 服务端强制+fail-closed、asynq+执行日志、审计链、L1 缓存必要性。两项目 README 均功能虚标——再证「以代码为准」 |
