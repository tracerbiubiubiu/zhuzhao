...
| 7 | **统一 PDP（广义，不以微服务拆分为前提）**（2026-09-20 补）：第三个服务需要复用同一套策略（activelist 行级自治不算），或合规强制策略集中审计/版本管理 | 策略散在 N 个服务进程内 = 语义漂移 + 审计碎片；对口形态 = 外置 PDP（Cerbos 带 Query Planner）；仍须先过 List 谓词适配与事实属性税两关 |
