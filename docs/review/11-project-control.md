...
| **RT-1** | RT 轮换重用检测：提交**非当前槽位**的签名有效 RT（含已轮换旧 RT）→ `GetDel` 删除该键（**连带使当前有效 RT 失效**）→ hash 不匹配 → 401；含多标签迟到提交误伤代价 | ✅ **设计内（2026-09-11 定性；2026-09-15 错误码状态拆分）**：旧 RT 被重放本身即盗用信号，拒绝并使当前会话失效、强制重登，符合 OAuth BCP / Auth0 / Google 的 reuse detection 最佳实践；作用域**单设备（用户+deviceId 单槽位）**，不跨设备、不涉数据正确性；多标签误伤属已知代价（触发驱动评估宽限窗口/family_id）。**对外码 2026-09-15 起三态分离**：重放 hash 不符 → **20015**、密码纪元不匹配（C3）→ **20014**、解析失败/空槽/账号状态（防枚举）→ 20004。详见 `docs/phase1/02-auth.md` §RT 轮换流程 |
