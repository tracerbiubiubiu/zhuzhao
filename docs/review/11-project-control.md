...
- **Phase 3 全部**：可观测性 / 多实例 / ~~审计 L2~~（✅ 2026-09-04 交付，见 §1 判定日志/审计归档行）/ 高可用 / 安全增强 / ops / **前端工程** / ~~activelist 集成~~（✅ M-A 2026-09-14 部署联调闭环，见 §1 网关反代号）— 暂缓项余可观测性/多实例/高可用/安全增强/ops/前端；执行结构 = **Wave W0–W4**（README §2.1.0，2026-08-31 确认）。**文档已全量就绪（2026-09-02）**：01 / 02 / 03 / 06 / 07 / 08 / 09 / 10（含 7-0 决议）/ 11 / 12 / 13-implementation-plan（执行计划）已编写；ops/deployment.md（B10）已补齐（见 §8 B9）
...
> **编号冲突已拍板（A2，2026-08-31）**：2b-ext 附件与 Phase 3 SLA 都曾规划 `000017`，规则 = **谁先启动谁占用，后者整体重排**。当前 **000017–000028 已占用**（000017/000018 = IW1/IW3，000019–000025 见上表，000026–000028 = 并发审查批次，下一编号 000029）；Phase 3 SLA（10-ticket-business §2 旧规划编号）启动时按此规则重排。
...
| **RT-1** | RT 轮换重用检测：提交**非当前槽位**的签名有效 RT（含已轮换旧 RT）→ `GetDel` 删除该键（**连带使当前有效 RT 失效**）→ hash 不匹配 → 401 + 20004；含多标签迟到提交误伤代价 | ✅ **设计内（2026-09-11 定性）**：旧 RT 被重放本身即盗用信号，拒绝并使当前会话失效、强制重登，符合 OAuth BCP / Auth0 / Google 的 reuse detection 最佳实践；作用域**单设备（用户+deviceId 单槽位）**，不跨设备、不涉数据正确性；多标签误伤属已知代价（触发驱动评估宽限窗口/family_id）。详见 `docs/phase1/02-auth.md` §RT 轮换流程 |
...
| B11 | **审计治理两件（2026-09-01 go-wind-admin 调研吸收）**：① **L2/L3 策略评估日志**——判定日志表 + `resource.Authorize`/`scope_resolver.resolve` 埋点（actor/资源/动作/scope 轴/结果/原因/trace_id），补 L2 拒绝无留痕盲区；② **审计归档**——audit_logs + 判定日志表超期导出 JSONL、导出成功后删行（保留期默认 180 天等保口径、可配置） | ①✅ **已实施（2026-09-04，E-①）**：迁移 000020 + EvalHook 埋点 + L2 writer（管道拍板 2026-09-03 异步）+ request_id 三列贯通；②✅ **已实施（2026-09-04，E-②/E-③）+ 手动链已实机贯通（2026-09-14 B+C：全链 succeeded、JSONL 双文件落盘、dead 复验过；联调修出 Dockerfile 非 root 目录缺陷）**；「按周期跑通」待 E 阶段（taskrunner cron 定义） |
