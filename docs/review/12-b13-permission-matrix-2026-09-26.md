...
>
> **基准**：2026-09-26，分支 phase4（565317b）。数据源=子代理全量勘察+路由表 dump 复核（**77 用户面端点**（auth 2+selfService 6+orgDelegated 9+biz 60）**+3 机器面**（探针 2+/internal/jobs/callback 1）+网关通配——26 批勘误：原 79 系含探针总数误作用户数），锚点均 file:line 实核。
>
...
*产出方法：2026-09-26 子代理全量勘察（router.go 77 用户面端点逐个对照三层代码锚点+迁移后 L1 绑定）+ 路由表 dump 计数复核（26 批）+ 盲区定性回填。本档为 point-in-time 快照，词表再动时重生。*
