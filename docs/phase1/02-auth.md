...
前端用 RT 请求 /auth/refresh
  │
  ├── 解析 RT，提取 userID + deviceId（失败/过期/篡改）→ 401 + 20004
  ├── 【防枚举】会话状态校验
  │   ├── EXISTS user:disabled:{userId} → 401 + 20004，不签发新 Token
  │   └── 或查 DB users.status != 启用 / 用户已删除 → 401 + 20004（与 RT 失效同一对外语义）
  ├── Lua 原子：GetDel 旧槽位 + GET user:pw_epoch:{userId} 密码纪元（C3）
  ├── 消费时刻权威校验（三态分离）：
  │   ├── 纪元 != RT claims.pwe → 401 + 20014（密码已修改/被重置）
  │   ├── 槽位为空 → 401 + 20004（登出/吊销/并发落败者先于成功者 SET）
  │   └── 槽位非空但 hash 不符 → 401 + 20015（槽位已是更新 RT，旧 RT 重现 = 重放信号 RT-1）
  ├── 签发新 AT + 新 RT（pwe 绑定消费时刻纪元）
  └── 存新 RT 到 Redis：refresh:{userId}:{deviceId}
```

> **RT 刷新**用 Lua `consumeRefreshLua` 原子完成「GetDel 旧槽位 + GET 密码纪元」（C3：两次读取必须原子，避免吊销与消费交错）；Phase 1 初版仅 GetDel，C3 引入纪元后合并进同一脚本。  
...
> **RT Reuse Detection（业界对照）**：Auth0 / OWASP ASVS 推荐的完整方案在 RT 被重用时不仅拒绝该 token，还应**吊销整个 token family**（同一次登录产生的所有 RT 链）。Phase 1 用 `GetDel` 原子读删：提交**非当前槽位**的签名有效 RT（含已轮换的旧 RT）→ `GetDel` 先原子删除该键（**连带使当前有效 RT 失效**）→ 再 hash 比对不匹配 → 返回 401 + 20015（2026-09-15 前为 20004，状态区分批拆出），但**不追踪 family**（同一设备的 RT key 只有一个槽位，旧 RT 自然不可用，但不跨设备链）。这是**有意设计**：旧 RT 被重放本身即盗用信号，正统做法就是拒绝并使当前槽位失效、强制重登；作用域为**单设备（用户 + deviceId 单槽位）**、**不跨设备**、**不涉数据正确性**，代价可接受（属**可接受的 Phase 1 安全级别**：单设备 RT 不可重用 + `user:disabled` 兜底）。**已知代价（2026-09-11 补登）**：多标签/迟到提交误伤——A 标签刷新成功后，B 标签携带已轮换旧 RT **迟到**提交（落在签发新 RT 与 `Set` 之间的极短窗口内）→ 当前有效键被删 → A、B 双双被迫重登；属**并发时序、非确定性**，业界已知。**演进选项**：Phase 2+ 若多标签体验问题真实发生，再行评估 ① **宽限窗口**（同一旧 RT 在短窗口内并发刷新放行一次；或改「先 GET 比对、命中才 DEL」——但**会削弱盗用检测**，须权衡）② 在 RT value 中嵌入 `family_id`，GetDel 命中后检查已用列表，发现重用则 DEL `refresh:{userId}:*` 全家桶。详见 [architecture §12.1 RT 并发刷新](../design/architecture.md#121-并发问题与应对)。
...
| JWT  malformed / 签名错 | 401 | 20003 | 否 | 登录状态已失效，请重新登录 | Info | 否 |
| JWT 过期 | 401 | 20002 | 否 | token 已过期 | Info | 否 |
| AT 在黑名单（已登出） | 401 | 20003 | 否 | 登录状态已失效，请重新登录 | Info | 否 |
...
| 旧 RT 失效（重放） | 使用已刷新过的旧 RT（槽位已推进） | 401 + 20015（`ErrRefreshTokenReplayed`） |
| **用户被禁用后 refresh** | 管理员禁用后，客户端仍用禁用前持有的 RT | **401 + 20004**，无新 AT/RT（RT 已 DEL；兜底查 user:disabled/DB；防枚举不外露禁用状态） |
| 并发双刷新（同时） | 同一 RT 同时两次 | 仅一次 200（落败者 401 + 20004 或 20015，时序决定；`GetDel` 原子读删保证互斥） |
| 迟到提交（多标签，已知代价） | A 刷新成功拿到新 RT 后，B 携带已轮换旧 RT 迟到提交 | B 401 + 20015；当前有效键被一并删除 → A、B 双双需重登（时序相关、非确定性，见 §RT 轮换流程） |
| 改密/重置密码后旧 RT | C3 密码纪元不匹配 | 401 + 20014（`ErrPasswordChanged`） |
