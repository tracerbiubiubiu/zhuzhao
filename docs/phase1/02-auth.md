...
| 并发双刷新（同时） | 同一 RT 同时两次 | 仅一次 200（另一次 401 + 20004；`GetDel` 原子读删保证互斥） |
| 迟到提交（多标签，已知代价） | A 刷新成功拿到新 RT 后，B 携带已轮换旧 RT 迟到提交 | B 401 + 20004；当前有效键被一并删除 → A、B 双双需重登（时序相关、非确定性，见 §RT 轮换流程） |
