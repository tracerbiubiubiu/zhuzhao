...
> **Step 10**，依赖 Step 4（middleware）。与 Step 6–9 **可并行**；Step 11 全量验收前挂载即可。

---

## 预期功能

> 权限码 SSOT：[07-menu §权限码命名规范](./07-menu.md#权限码命名规范与-api-对齐ssot)。Phase 1 **无**审计菜单种子，下列码供 Phase 2+ 预留；当前仅 admin/superadmin Casbin 通配可访问。

| 功能 | 场景 | API / 说明 | 权限码 |
|------|------|------------|--------|
| 操作日志中间件 | 自动记录所有 API 请求 | 同步写入 DB，不阻塞响应（见下方分析） | — |
| 日志查询 | 管理员查看操作日志 | `GET /api/v1/audit/logs` | `audit:list` |
| 日志筛选 | 按用户、操作类型、时间范围筛选 | query string 参数 | `audit:list` |
