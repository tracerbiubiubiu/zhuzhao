...
> ⚠ 2026-09-22 虚标审计勘误（**曾被 25bda2e 误回退，本轮恢复——见 phase4/02 §5.5 行 25**）：算法切换构造器（NewHS256Manager/NewRS256Manager/method 字段）未落码——实况=utils jwt.Manager 仅 HS256 硬编码；**pin 算法防 key-confusion 属实**（验签强制 HMAC）。RS256/JWKS 维持 roadmap 预留条件触发。
...
```go
// ⚠ 2026-09-22 虚标审计勘误（曾被 25bda2e 误回退，本轮恢复）：本接口未落码——全仓无 ResourceAuthorizer；实际接缝 = internal/pkg/resource/registry.go 的 Resource(Authorize/GetFilter)+Registry（Phase 2+）
// ~~Phase 1：接口定义~~（示意稿）
...
> ⚠ 2026-09-22 虚标审计勘误（曾被 25bda2e 误回退，本轮恢复）：`UserQueryService` 接口未落码（全仓零命中）；且本节 gRPC 方向已被 2026-09-03 拍板（HTTP+JSON，gRPC 不引入，standards §2）覆盖——以下为历史设计稿。
...
| 跨服务统一可见性 | 一列表页聚合多属主数据按同一套可见性过滤；再升跨资源关系链 | 编排层逐服务取回合并 → 触发表「跨资源关系链」（#1）或「微服务拆分后的统一 PDP/语义漂移」（#5）命中 → ReBAC 评估（ResourceAuthorizer 接缝） |
