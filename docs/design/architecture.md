...
- 属主判断（L3）不短路：先过 L2 可见性，属主命中不豁免 L2（对齐 02/11 决策路径 A：L2 在前、属主不短路）
...
| 场景 | 问题描述 | 应对方案 |
|------|----------|----------|
| **RT 并发刷新** | 多标签页同时用同一 RT 刷新，轮换后只有第一个成功 | 前端加请求防抖；后端 Lua 原子 `GetDel`+纪元双取轮换，落败者返回 **401 + 20004**（空槽）或 **20015**（新槽已写，重放语义），时序决定（见 [phase1/02-auth §Token 刷新](../phase1/02-auth.md#token-刷新)） |
