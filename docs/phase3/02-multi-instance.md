...
> **状态**：已编写（2026-08-31，基于并发/多实例审计 + eiam 参考实现）｜ **Wave 归属**：~~W1 可运维基座~~ **M1（🚦 部署形态升级为多实例时启动，2026-09-02 §22.1 修订）**｜ 对应 Step 2
> ~~**前置**：W0 清零（检查单 A 档）。**本模块是 W2 的硬前置**（提案已确认，README §2.1.0）~~ **已被 §22.1/§23 修订**：现行主链无「M1→M2」链式依赖——事件基建（M-E）独立于多实例；本文价值 = 触发后实施方案 + **防重约定**（`sla:scan` Unique + L1 advisory lock，写码时按本文执行，对事件消费适用）。
