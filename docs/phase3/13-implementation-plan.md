...
> **状态**：**规划稿（2026-09-02 建档）**。**2026-09-15 项目内工程收口（口径 A，所有者拍板）**：M-E ✅ 全收官（taskrunner 容器化部署 + 回调链联调 + audit_archive 按周期跑通，2026-09-14 联调五阶段 A–E）、M-A ✅ 全收官（网关化 + 双栈部署 + 阶段 A 收尾四件）、M-HR 按所有者拍板 = **预留接口即可**（~~HRFetcher 在位~~ **⚠ 2026-09-22 勘误：接口未落码**——全仓无 `HRFetcher` 类型（grep 实证，internal/ 无 hr 目录），原「在位/完成」系虚标；真实对接时接口+引擎+adapter 一并开工，真实对接时启动并另拍三项）、M-SSO / M-Mig 🚦 **外部前置待触发**（进内网/迁移窗口，非工程项）；终验：四档 acceptance（27+66+26+34）FAIL=0 + 四仓全门禁绿（2026-09-15 收口终验，报告见 [deliverables](../../deliverables/software-company/phase3-closure-report-2026-09-15.md)）。本文档回答「怎么做」与现行进度。
...
| 迁移 | Phase 3 迁移编号启动时按 A2 规则重排（当前 000001–000025 已占用，下一编号 **000026**；000020–000023 = 判定日志/job_submissions/任务菜单/params 快照列，000024/000025 = activelist 权限面/audit_menu） |
...
| **M-E 事件与任务总线** | ① Asynq 底座（Scheduler/PeriodicTask/worker/重试/超时/阻塞策略按任务拍板）；② **预置动作 = 回调 zhuzhao 内网端点**（业务 handler 在 zhuzhao：审计归档 B11② 首个、通知、SLA 扫描、外部回调等按需）；③ 业务点**显式发布**（zhuzhao Enqueue/API 提交 → taskrunner 异步触发回调）；~~自定义脚本任务~~ **2026-09-03 降级 🚦**（上传 python/shell 暂不需要；Dagu/自研调研见 [15](./15-script-platform-dagu-vs-inhouse.md)，按需再启）；**独立仓库 + 独立部署 + 独立 Redis**（2026-09-03 拍板，形态见 §1 注记）；**权限前置（2026-09-04 校准降级触发驱动）**：~~平台策略库批次 A 2–3 天~~ 预设消费方随落地清零（E-④ 走 L1 权限码 / E-② 走 AK/SK 验签 / E-⑤ 参数级过滤），触发条件 = zhuzhao 自有新资源需要 L2（16 号 §3 批次 A 行）；**zhuzhao 侧配套细排（内网回调端点体系 / 任务管理代理 / 部门可见性 / 提交日志，E1–E6）见 [16-external-integration](./16-external-integration.md)，约 5–7 人日与 taskrunner 侧并行**；**服务间 AK/SK HMAC 签名**（utils `aksk` 包 C8 先行，zhuzhao 回调端点验签 / client 签名，16 号 §9 基线，2026-09-03 拍板）；**zhuzhao 侧配套完成态（2026-09-07 回填）**：批次 A 降触发驱动、C8 ✅、E-① 判定日志 ✅（000020）、E-② 回调基建 ✅（000021）、E-③ audit_archive ✅、E-④ 任务管理代理 ✅（000022）、E-⑤ 简化全员可见（策略表 🚦 编号暂不占用）、E-⑦ 契约改造 ✅——**✅ M-E 全收官（2026-09-14/15：taskrunner 容器化部署 + 回调链联调 + audit_archive 按周期跑通）**；**目标架构注记镜像（2026-09-07，taskrunner.md §2/§4 已入档）**：zhuzhao 演进方向 = **API 网关 + IAM**（薄网关，不持业务能力），动作归属泛化「能力属主服务」——各服务挂自己的动作端点、taskrunner 统一调度（当前预置 handler 仍在 zhuzhao = zhuzhao 自己的能力，薄化随业务迁移演进；owner_service/多 credential 已预留）；**E-⑦ 契约前置**（taskrunner C10 API 约定改造 + C11 runs dept 过滤/task 响应补 dept，M3 契约冻结前）✅ 已实施 | 3–4（⚠️ 待校准；原 6–8） | 归档任务按周期跑通；预置动作 触发→回调执行→失败重试 闭环；全门禁绿 |
| **M-A activelist 独立实现** | 独立库 + 独立数据库（§22.3/§23）；**外部事件接入契约由 activelist 侧定义**（工单非首数据源，§23.2）；**2026-09-03 职责收敛 + 需求澄清**（ADR-003 修订）：收窄为**动态数据模型薄层**——任意自定义类型（字段=`int`/`string`/列表）、无用户认证（用户侧收敛 zhuzhao 网关；**服务间 AK/SK 验签**见批次 B）、查询=仅 id 分页+时间倒序、PG **每类型表 + `data` JSONB**（id 自增/乐观锁/软删保留）、导入导出 JSON（**幂等=全量替换**：单事务清表重插、保留源 id、不需要业务唯一键——方案 D 定稿同步，2026-09-03）；事件与审计移交 zhuzhao（Asynq 显式发布 / zhuzhao 侧记录），进程 3→1；独立部署保留；**前置：~~共享 utils 抽取~~ ✅ 已完成（zhuzhao-utils v0.2.0 已发布并 pin，9 包含 logger/postgres，2026-09-03 发布 / 09-08 去 replace 升级）**；**权限前置（2026-09-03 落档）**：网关化批次（design-decisions §25.5 批次 B，~1 周）**✅ 已实施并部署联调闭环（2026-09-09 代码收尾 2b29b6e；2026-09-14 阶段 A 部署批+联调闭环，见 [16 号 §4](./16-external-integration.md)）**——反代核心/身份断言（明文 X-Operator 入 AK/SK 签名覆盖，B2 已关）/API 级限流/Restrict 资源开关/API 入 menu_apis（000024）＋**BK-22 路由↔menu_apis 启动期对账**（review/11 §8 B14 ✅）/审计跳 body（ADR-003 G4 蓝图保留）；**zhuzhao 侧配套细排（D1–D5 对照 + D3 审计落点）见 [16-external-integration](./16-external-integration.md)**；**✅ M-A 全收官（2026-09-14/15）** | 约 1.5–3（澄清后估算 ⚠️ 待校准；原单估） | 按其项目自身验收 |
| **M-HR HR 同步（预留接口版）**（**2026-09-14 所有者拍板：预留接口即可，真实对接时启动**） | `HRFetcher` 接口（~~在位~~ **⚠ 2026-09-22 勘误：接口未落码**——全仓无 `HRFetcher` 类型（grep 实证，internal/ 无 hr 目录），原「在位/完成」系虚标；真实对接时接口+引擎+adapter 一并开工）+ sync 引擎 + 本地 mock adapter（内网 adapter 即插）——即三件均未开工；启动时拍板：离职处置/部门撤销级联/跨部门权限分配规则 | 3–5（🚦 真实对接时） | 组织同步三规则对 mock 源跑通；对账幂等；三拍板项落档 |
...
| **L1 事件升级**（[10 §7](./10-ticket-business.md)） | 2–3 | 2c + Asynq 底座（M2 内） | 迁移编号按 A2 启动时重排（原规划 ~~000024~~ 已被 activelist 权限面占用，启动时取下一可用号，现口径 **000026**）——ticket_events ALTER event_type/processed；单消费者轮询 + 分布式锁（advisory lock 防重，按 02 号约定）；audit/signal 双职责（决议 5 定死两条记录） |
| **Asynq 底座**（ADR-002） | 1–2 | L1 | Scheduler + PeriodicTask + worker；铁律：L1 管事件持久化、Asynq 管执行；sla:scan 配 Unique 去重 |
| **7a SLA**（[10 §2](./10-ticket-business.md)） | 3–5 | L1+Asynq | ticket_sla + sla_policies（迁移编号启动时按 A2 重排）；扫描→sla:breach 链；**四必坑**（暂停态/提前取消/幂等/Enqueue 原子性）→ TB12–15 |
| **7b 通知**（[10 §3](./10-ticket-business.md)） | 3–4 | L1+Asynq | notifications 表（编号同上 A2 重排）+ 4 API + SMTP + 邮件矩阵；「主管」= 所属组织 owner_user_ids |
| **7c 审批流引擎**（[10 §4](./10-ticket-business.md)） | **8–12** | 2c + L1 | BranchedStateEngine（**硬交付**）+ workflow_* 表（编号同上 A2 重排）+ 发布快照表（决议 4）+ CanApproveNode + 驳回语义 + 发起人撤回（TB16）；引擎本体最重 |
| **7d 分派**（[10 §5](./10-ticket-business.md)） | 2–3 | 2c | assignment_rules（编号同上 A2 重排）+ 匹配引擎 + 五项决议；分派失败不回滚创建 |
...
> **HR 同步**：**已升主链（2026-09-02 拍板，design-decisions §22.2/§23.2）**——见 **M-HR** 里程碑（预留接口版：HRFetcher + 引擎 + mock adapter；**⚠ 2026-09-22 勘误：接口未落码**——全仓无 `HRFetcher` 类型（grep 实证，internal/ 无 hr 目录），原「在位/完成」系虚标；真实对接时接口+引擎+adapter 一并开工）；组织架构来自公司接口定时更新为已确认真实诉求，且为**内网迁移前置**。**附件 / auth-enhance**（2b-ext）：仍独立窗口按需；附件启动会占用迁移号（⚠️ 与 Phase 3 竞争，A2 规则）。
...
| 日期 | 说明 |
|---|---|
| 2026-09-09（BK-22 排期点名） | M-A 权限前置段点名 **BK-22 路由↔menu_apis 启动期对账**（~半天随批次 B；review/11 §8 B14 在册）——消除「B14 在册而排期 SSOT 未点名」的级联缺口 |
