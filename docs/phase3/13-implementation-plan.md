...
| **M-A activelist 独立实现** | 独立库 + 独立数据库（§22.3/§23）；**外部事件接入契约由 activelist 侧定义**（工单非首数据源，§23.2）；核心功能人日单估 | 单估 | 按其项目自身验收 |
...
> **合计（现行主链，不含 🚦/单估）**：约 **10–15 人日** = M0 1–2 + M-E 6–8 + M-HR 3–5（M-A 核心功能**单估**、M-Mig 🚦、M1 6–10 🚦 均未计入）。量级参考：审计归档/脚本任务参照 ginfast scheduler 执行器（design-decisions §23 补充拍板已核验可用）。
...
> ⚠️ **不确定项**：① 工单审批/报表前端已随 §23 暂缓（仅 BK-18 管理页/动态表单仍为 IW3 独立窗口，见 [12-frontend](./12-frontend.md)）；② M-A activelist 核心功能人日**单估**（按其项目自身节奏）；③ 迁移号是否需为附件让位。
