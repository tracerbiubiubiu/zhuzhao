...
| **M-HR HR 同步（预留接口版）**（**2026-09-14 所有者拍板：预留接口即可，真实对接时启动**） | `HRFetcher` 接口（在位）+ sync 引擎 + 本地 mock adapter（内网 adapter 即插）；启动时拍板：离职处置/部门撤销级联/跨部门权限分配规则 | 3–5（🚦 真实对接时） | 组织同步三规则对 mock 源跑通；对账幂等；三拍板项落档 |
