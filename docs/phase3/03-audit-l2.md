...
| 2026-09-14 | **B11②「按周期跑通」闭环（E 阶段，联调五阶段收官）**：audit_archive cron 任务定义入 taskrunner（验证用每分钟→收敛每日 02:30 `30 2 * * *`）；首轮周期触发 succeeded；**AL5 真数据实证**——预置 200 天前审计行（id 1228）被 cron run 导出 JSONL（全字段）并从 audit_logs 删除；AL6 维持集成测试承载（保留期 180 天默认/config/params 三级）。部署形态：taskrunner 容器化（deploy/compose.yaml，C3 网络隔离零宿主端口），zhuzhao↔taskrunner 经共享网服务名互寻；故障演练四项全过（队列续跑/PG 自愈/dead 重驱/SIGTERM 排空，见 00 §5 E 行） |
