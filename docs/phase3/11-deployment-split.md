...
> 创建日期：2026-08-25。
> **⚠ 重定位注记（2026-09-02 §23）**：工单自研暂缓、Phase 2 封版——本文**整体转 🚦 参考**：档位 2 触发条件随部署形态（13 号 §7）；工单部署形态（含 Phase 2 资产处置）= 迁移时拍板（§23.3）。独立部署的外部服务（taskrunner/activelist）不经本文档路径（它们是独立仓库/库，非「同二进制模块选择启动」）。
