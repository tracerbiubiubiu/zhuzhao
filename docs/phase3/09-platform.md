...
- 出现机器到机器（M2M）调用方（~~如 activelist 对 zhuzhao 的调用~~ **2026-09-03 收敛后 activelist 用户侧零权限经网关被调，不再主动调 zhuzhao**；场景 = 外部系统对接、非内网调用），需要非人类凭据。
...
- **设计输入（2026-09-08 注记）**：业界蓝本 = GitHub fine-grained PAT——每把 key 绑**权限码子集**（scope 字段管用，最小权限），而非仅服务身份；首版可只做服务身份，per-key 随真实外部调用方的最小权限诉求启用（design-decisions §26.5）。
