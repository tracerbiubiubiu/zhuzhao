...
RM=$(psql_q "SELECT COUNT(DISTINCT rm.menu_id) FROM role_menus rm
  ...
  WHERE r.code IN ('superadmin','admin')
    AND m.code IN (
      ...
      'ticket_update_btn','ticket_close_btn','ticket_assign_btn',
      'ticket_delete_btn','ticket_comment_btn','ticket_note_btn')")
check "role_menus ticket admin binds" "1" "$([ "$RM" -ge 10 ] && echo 1 || echo 0)"
...
TECH_ID=$(echo "$TECH_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('id','0'))")
...
make_user() {
  ...
  resp=$(curl -s -X POST "$BASE/users" \
    -H "Authorization: Bearer $SAT" -H 'Content-Type: application/json' \
    -d "$payload")
  echo "$resp" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('id','0'))"
}
...
login_user() {
  ...
  echo "$resp" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('access_token',''))"
}
AAT=$(login_user "a_$$")
BAT=$(login_user "b_$$")
VAT=$(login_user "v_$$")
...
TID_A=$(echo "$T1" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('id','0'))")
...
TPL_PRI=$(echo "$T2_TPL" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('priority','0'))")
...
TID_B=$(echo "$T_B" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('id','0'))")
...
T4_T=$(echo "$T4" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('title',''))")
...
R7_TITLE=$(echo "$R7" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('title',''))")
...
C1_ID=$(echo "$C1" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('id','0'))")
...
# ========================================================================
# 工单关联（§3 前移功能）：建立关联须对 source/target 都走 L2/L3 read 鉴权
# ========================================================================
# 额外创建 A 的第二张工单便于关联
TID_A2_JSON=$(curl -s -X POST "$BASE/tickets" \
  -H "Authorization: Bearer $AAT" -H 'Content-Type: application/json' \
  -d "{\"type_code\":\"incident\",\"title\":\"关联用工单 A2\",\"priority\":3,\"org_id\":$TECH_ID}")
TID_A2=$(echo "$TID_A2_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('id','0'))")

# A 建立 TID_A → TID_A2 的关联（都是 A 创建的，read 鉴权均通过）
...
FE_ID=$(echo "$FE_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('id','0'))")
...
FE_TID=$(echo "$FE_TICKET" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('id','0'))")
