...
// C3 回归：改密后旧 RT 必拒（纪元比对），新口令登录的 RT 正常刷新。
func TestAuthService_PasswordChangeInvalidatesOldRefreshToken(t *testing.T) {
	...
	// 改密（触发全量吊销 + 纪元 INCR + 重签当前设备 pair）
	newPair, err := authSvc.UpdatePassword(ctx, 1, "admin123", "NewPass456", pair.AccessToken, "dev-1")
	require.NoError(t, err)

	// 旧 RT（改密前签发，pwe=0）刷新 → 纪元 0 ≠ 当前 1 → 拒
	_, err = authSvc.Refresh(ctx, pair.RefreshToken)
	require.Error(t, err)
	var biz *errcode.Error
	require.ErrorAs(t, err, &biz)
