...
| ~~B-3~~ | ~~`policy_evaluation_logs` 零索引~~（⚠ 2026-09-22 核验勘误：**误报**——000020:20-22 实有 idx_pel_created/idx_pel_actor_created/idx_pel_trace 三索引，见 phase4/02 §5.5 行 25⑥） | 误报 | 性能 |
...
3. ~~**`policy_evaluation_logs` 是否已有读 API**~~（读 API 已核不存在——十三批勘察结论；且 B-3 本身系误报，见勘误）。
